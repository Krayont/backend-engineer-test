...
//
const creditAddress = async (address: string, value: number, blockId: string, height: number, txId: string, txIndex: number, client: PoolClient): Promise<void> => {
  const query = `
...
//
const debitAddress = async (address: string, value: number, blockId: string, height: number, txId: string, txIndex: number, client: PoolClient): Promise<void> => {
  const query = `
    INSERT INTO ledgers (address, value, block_id, height, transaction_id, index, type)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
  `;
...
//
const deleteByHeight = async (height: number, client: PoolClient): Promise<void> => {
  //
  const query = `
    UPDATE ledgers
    SET
